...
def arg_parsing(argv):
    ...
    for opt, arg in opts:
        ...
        if opt == "-D":
            duplication = arg
            duplication_list = arg.split(',')
        if opt == "-T":
            transfer = arg
            transfer_list = arg.split(',')
        if opt == "-L":
            loss = arg
            loss_list = arg.split(',')
        if opt == "--seed":
    ...
    if not len(duplication_list) == len(transfer_list) == len(loss_list):
        raise IOError(
            'The number of values for event costs must be consistent.')
# ------------------------------------------------------------------------------
...
def run_OptRoot(argv):
    ...
    if "-D" in input_args:
        input_args.remove("-D")
        input_args.remove(duplication)
    input_args += ["-D", duplication_list[0]]

    if "-T" in input_args:
        input_args.remove("-T")
        input_args.remove(transfer)
    input_args += ["-T", transfer_list[0]]

    if "-L" in input_args:
        input_args.remove("-L")
        input_args.remove(loss)
    input_args += ["-L", loss_list[0]]
    ...
    if dated:
        ...
        for i in range(len(duplication_list)):
            D_index = input_args.index("-D")
            del input_args[D_index]
            del input_args[D_index]
            input_args += ["-D", duplication_list[i]]
            T_index = input_args.index("-T")
            del input_args[T_index]
            del input_args[T_index]
            input_args += ["-T", transfer_list[i]]
            L_index = input_args.index("-L")
            del input_args[L_index]
            del input_args[L_index]
            input_args += ["-L", loss_list[i]]
            new_output_file = output_file + "_" + str(i + 1)
            o_index = input_args.index("-o")
            del input_args[o_index]
            del input_args[o_index]
            input_args += ["-o", new_output_file]
...
def run_RangerDTL():
    ...
    open(all_optimal_rootings, 'w').close()

    for i in range(len(duplication_list)):
        new_output_file = output_file + "_" + str(i + 1)
        OptRoot_output = dir_addr + "OptRoot_output" + "_" + str(i + 1)
        shutil.move(new_output_file, OptRoot_output)
        with open(OptRoot_output) as infile, \
                open(all_optimal_rootings, "r+") as outfile:
            for line in infile:
                if line.startswith("("):
                    copy = True
    ...
    with open(all_optimal_rootings) as infile:
        num_optrootings = len(infile.readlines())

    for i in range(num_optrootings):
        name = "input"
        ...
        with open(all_optimal_rootings) as infile, \
            ...
            for line in islice(infile, i, i + 1):
                outfile.write(line)
    ...
    input_args.remove(new_output_file)
    if "-o" in input_args:
        input_args.remove("-o")
    else:
            ...
            for i in range(1, sample + 1):
                ...
                if dated:
                    ...
                    if os_name.startswith("linux"):
                        ...
                        D_index = input_args2.index("-D")
                        del input_args2[D_index]
                        del input_args2[D_index]
                        input_args2 += ["-D",
                                        duplication_list[(i - 1) % numcosts]]
                        T_index = input_args2.index("-T")
                        del input_args2[T_index]
                        del input_args2[T_index]
                        input_args2 += ["-T",
                                        transfer_list[(i - 1) % numcosts]]
                        L_index = input_args2.index("-L")
                        del input_args2[L_index]
                        del input_args2[L_index]
                        input_args2 += ["-L", loss_list[(i - 1) % numcosts]]
...
# Runs AggregateRanger_edit:


def run_AggregateRanger_edit():
    ...
    for item in sorted(os.listdir(directory)):
        if item.startswith("output"):
            ...
            if os_name.startswith("linux"):
                aggr = subprocess.Popen("../../AggregateRanger_recipient.linux out".split(),
                                        stdout=subprocess.PIPE)

            elif os_name.startswith("win32"):
                aggr = subprocess.Popen("../../AggregateRanger_recipient.win out".split(),
                                        stdout=subprocess.PIPE)

            else:
                aggr = subprocess.Popen("../../AggregateRanger_recipient.mac out".split(),
                                        stdout=subprocess.PIPE)
...
def generate_output():
    ...
    with open(dir_addr + "all_optimal_rootings") as infile, \
        ...
        outfile.write(("Input file: " +
                       ...
                       str(sample) +
                       "\n"))
        outfile.write("\n------------ All Optimal Rootings for Gene Tree Across all Event Cost Sets -------------\n")
        for line in infile:
            outfile.write(line)
        infile.seek(0, 0)
        outfile.write("The total number of optimal rootings is: " +
                      str(len(infile.readlines())))
        outfile.write(("\n" +
                       "\n" +
    ...
    if not summary:
        shutil.copyfile(dir_addr + "final_output",
                        output_file + "_long")
    # create species_tree_label file
    with open(dir_addr + "output1/out1") as infile, \
            open(dir_addr + "tempOptRootOut_species_tree_label", "w") as outfile:
        for line in infile:
